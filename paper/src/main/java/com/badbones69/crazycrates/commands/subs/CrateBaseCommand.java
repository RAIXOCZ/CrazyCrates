    ...
    public void onAdminTeleport(Player player, @Suggestion("locations") String id) {
        ...
        for (String name : FileManager.Files.LOCATIONS.getFile().getConfigurationSection("Locations").getKeys(false)) {
            if (name.equalsIgnoreCase(id)) {
                ...
                int xCoord = FileManager.Files.LOCATIONS.getFile().getInt("Locations." + name + ".X");
                int yCoord = FileManager.Files.LOCATIONS.getFile().getInt("Locations." + name + ".Y");
                int zCoord = FileManager.Files.LOCATIONS.getFile().getInt("Locations." + name + ".Z");
    ...
    public void onAdminCrateGive(CommandSender sender, @Suggestion("key-types") String keyType, @Suggestion("crates") String crateName, @Suggestion("numbers") int amount, @Suggestion("online-players") Player target) {
        ...
        if (crate.getCrateType() != CrateType.MENU) {
            ...
            if (!event.isCancelled()) {
                ...
                eventLogger.logKeyEvent(target, sender, crate, type, EventLogger.KeyEventType.KEY_EVENT_GIVEN, logFile, logConsole);
            }
